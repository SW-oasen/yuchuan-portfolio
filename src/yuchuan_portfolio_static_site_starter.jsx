...
import { Github, ExternalLink, Mail, PlayCircle, FileText, Eye } from "lucide-react";
      ...
      <Card className="rounded-2xl shadow-md hover:shadow-lg transition-shadow">
        <CardContent className="p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h3 className="text-xl font-semibold">{p.title} <span className="text-sm text-muted-foreground font-normal">• {p.date}</span></h3>
              <p className="mt-2 text-sm md:text-base text-muted-foreground">{p.summary}</p>
            </div>
            <div className="flex gap-2 flex-wrap">
              {p.key && (
                <Button variant="default" size="sm" asChild>
                  <a href={`#/project/${p.key}`}>
